...
import { Github, Linkedin, Mail, MapPin, ChevronDown, Twitter, Pen } from 'lucide-react';
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center pt-16 overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-[0.08] dark:opacity-[0.12]">
        <div className="absolute top-24 left-4 sm:left-10 font-mono text-[10px] sm:text-xs text-slate-900 dark:text-slate-100 whitespace-pre animate-float">
{`class MLEngineer:
    def __init__(self):
        self.skills = []
        self.experience = 5.5`}
        </div>
        <div className="absolute top-52 sm:top-48 right-4 sm:right-16 font-mono text-[10px] sm:text-xs text-slate-900 dark:text-slate-100 whitespace-pre animate-float-delayed-1">
{`import tensorflow as tf
model = tf.keras.Sequential()
model.add(tf.keras.layers.Dense(128))`}
        </div>
        <div className="hidden md:block absolute top-[60%] left-[15%] font-mono text-xs text-slate-900 dark:text-slate-100 whitespace-pre animate-float-delayed-2">
{`@app.post("/predict")
async def predict(data: Input):
    return model.predict(data)`}
        </div>
        <div className="absolute bottom-32 left-4 sm:left-24 font-mono text-[10px] sm:text-xs text-slate-900 dark:text-slate-100 whitespace-pre animate-float-delayed-3">
{`pipeline = Pipeline([
    ('scaler', StandardScaler()),
    ('model', XGBClassifier())
])`}
        </div>
        <div className="absolute bottom-48 right-4 sm:right-24 font-mono text-[10px] sm:text-xs text-slate-900 dark:text-slate-100 whitespace-pre animate-float">
{`def deploy_model(model):
    return model.serve()`}
        </div>
        <div className="hidden md:block absolute top-[35%] right-[10%] font-mono text-xs text-slate-900 dark:text-slate-100 whitespace-pre animate-float-delayed-1">
{`docker build -t ml-api .
kubectl apply -f deployment.yaml`}
        </div>
        <div className="hidden md:block absolute top-[70%] right-[20%] font-mono text-xs text-slate-900 dark:text-slate-100 whitespace-pre animate-float-delayed-2">
{`metrics = evaluate_model(
    y_test, predictions
)`}
        </div>
        <div className="hidden md:block absolute bottom-[60%] left-[5%] font-mono text-xs text-slate-900 dark:text-slate-100 whitespace-pre animate-float-delayed-3">
